...
class CoreRouterSpec extends PlaySpec {
  ...
  "CoreRouter" should {
    ...
    "serve API specs" in {
      ...
      val docsRequest = FakeRequest(GET, "/specs?url=/specs.yml")
      val Some(docs) = route(application , docsRequest)
      status(docs) mustBe Status.OK

      val indexRequest = FakeRequest(GET, "/specs/index.html?url=/specs.yml")
      val Some(index) = route(application , indexRequest)
      status(index) mustBe Status.OK

      val swaggerUIRequest = FakeRequest(GET, "/specs/swagger-ui-bundle.js")
      val Some(swaggerUI) = route(application , swaggerUIRequest)
      status(swaggerUI) mustBe Status.OK
    }
    "redirect request to API specs when url parameter is missing" in {
      val redirectDocsRequest = FakeRequest(GET, "/specs")
      val Some(redirectDocs) = route(application, redirectDocsRequest)
      status(redirectDocs) mustBe Status.SEE_OTHER

      val redirectIndexRequest = FakeRequest(GET, "/specs/index.html")
